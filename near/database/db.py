...
def connect_to_postgres():
    return psycopg2.connect(**__CONFIG)
...
class Embeds:
    ...
    @staticmethod
    def __create_table_embed(cursor):
...
def setup():
    try:
        ...
        if connection:
            cursor = connection.cursor()
            Embeds.__create_table_embed(cursor=cursor)
            Embeds.__setup_table_embed()
